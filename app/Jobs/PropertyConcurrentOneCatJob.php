...
use App\Models\ModelPropertyOneCatJob;
...
class PropertyConcurrentOneCatJob implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        foreach ($allGroupDate as $keyallGroupDate => $valueallGroupDate) {
			...
			{
				$check = ModelPropertyOneCatJob::where('date_from', $keyallGroupDate)
					->where('property_id', env("PROPERTY_ID"))
					->first();
	
				if($check) {
					ModelPropertyOneCatJob::where('id', $check->id)->firstorfail()->delete();
				}
				$model = new ModelPropertyOneCatJob();
